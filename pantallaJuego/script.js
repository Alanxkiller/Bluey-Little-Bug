...
function iniciar() {
  ...
  setInterval(function () {
    ...
    f.load().then(function (font) {
      ...
      ctxCron.fillStyle = "white";
      ctxCron.textAlign = "center";
      ctxCron.fillText(seg, 1250, 30);

      document.fonts.add(font);
      ctxCron.font = "40px Font name";
      ctxCron.fillStyle = "white";
      ctxCron.textAlign = "center";
      ctxCron.fillText(min, 1200, 30);
    })
  }, 1000);
...
function finalizado(e) {
  if (ganar == 6) {
    ...
    localStorage.actualPoints = puntos;
    localStorage.actualTime = segs;
    guardarDatos();
    setTimeout(()=>{
      window.location.href = "../pantallaFelicidades/congrats.html"
    }, 2000)
  }
  elemento = e.target;
...
function guardarDatos(){
  ...
  let newData = JSON.stringify({
    nombre: localStorage.actualPlayer,
    puntos: localStorage.actualPoints,
    tiempo: localStorage.actualTime
  })
  ...
  for(let i in playerNames){
    ...
    if(localStorage.actualPlayer == player.nombre){
      if(player.tiempo == 0){
        playerNames[i] = newData;
      }else if(parseInt(localStorage.actualTime) < parseInt(player.tiempo)){
        playerNames[i] = newData;
      }else{
        console.log("a");
      }
...
document.addEventListener("click", (e)=>{
  if(e.target.matches("#music")){
    const music = document.getElementById("music");
    if(playMusic == false){
      playMusic = true;
      music.setAttribute("src", "btnMusica/VolON.png");
      ...
      playMusic = false;
      music.setAttribute("src", "btnMusica/VolOFF.png");
